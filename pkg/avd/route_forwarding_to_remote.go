...
func (s *Server) AddRouteForwardingToRemote(
	ctx context.Context,
	...
	logger.Debugf(ctx, "AddRouteForwardingToRemote(ctx, '%s', '%s', '%s')", sourcePath, dstURL, getRouteMode)
	defer func() {
		logger.Debugf(ctx, "/AddRouteForwardingToRemote(ctx, '%s', '%s', '%s'): %v %v", sourcePath, dstURL, getRouteMode, _ret, _err)
	}()
	...
	input, err := s.Router.GetRoute(ctx, sourcePath, getRouteMode)
	...
	fwd.StreamForwarder, err = newStreamForwarder(ctx, fwd.Input.Node, fwd.Output, recoderConfig)
	if err != nil {
