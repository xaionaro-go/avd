...
type ListeningPortRTMPConsumers struct {
	Server            *Server
	Listener          net.Listener
	ConnectionsLocker xsync.Mutex
	Connections       map[net.Addr]*ConnectionRTMPConsumer
}

func (p *ListeningPortRTMPConsumers) StartListening(ctx context.Context) error {
...
func (p *ListeningPortRTMPConsumers) Close(ctx context.Context) error {
	p.Listener.Close()
	return nil
}

func (p *ListeningPortRTMPConsumers) Listen(ctx context.Context) error {
	...
	for {
		...
		rtmpConn, err := newConnectionRTMPConsumer(ctx, p, conn)
		if err != nil {
